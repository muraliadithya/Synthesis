;Datatype of images
(declare-datatypes () ((Img
 s3 s2 s1 t2 t3 t1
)))
;Constant symbols for objects
;All values are distinct by default
(declare-datatypes () ((Obj
 s3_o1 s3_o2
 s2_o2 s2_o1
 s1_o18 s1_o13 s1_o12 s1_o11 s1_o10 s1_o17 s1_o16 s1_o15 s1_o14 s1_o9 s1_o8 s1_o3 s1_o2 s1_o1 s1_o7 s1_o6 s1_o5 s1_o4
 t2_o5 t2_o4 t2_o7 t2_o6 t2_o1 t2_o3 t2_o2 t2_o9 t2_o8 t2_o15 t2_o14 t2_o11 t2_o10 t2_o13 t2_o12
 t3_o4 t3_o2 t3_o3 t3_o1
 t1_o1 t1_o2 t1_o3 t1_o4 t1_o5 t1_o6 t1_o7 t1_o8 t1_o9 t1_o12 t1_o13 t1_o10 t1_o11 t1_o16 t1_o14 t1_o15
;extraneous objects
 person car backpack suitcase nullobj truck bus handbag
)))
;Membership of objects in images
;Extraneous objects belong to no images
(define-fun inImg ((x Obj )(y Img)) Bool
(or
(and (= x s3_o1) (= y s3))
(and (= x s3_o2) (= y s3))
(and (= x s2_o2) (= y s2))
(and (= x s2_o1) (= y s2))
(and (= x s1_o18) (= y s1))
(and (= x s1_o13) (= y s1))
(and (= x s1_o12) (= y s1))
(and (= x s1_o11) (= y s1))
(and (= x s1_o10) (= y s1))
(and (= x s1_o17) (= y s1))
(and (= x s1_o16) (= y s1))
(and (= x s1_o15) (= y s1))
(and (= x s1_o14) (= y s1))
(and (= x s1_o9) (= y s1))
(and (= x s1_o8) (= y s1))
(and (= x s1_o3) (= y s1))
(and (= x s1_o2) (= y s1))
(and (= x s1_o1) (= y s1))
(and (= x s1_o7) (= y s1))
(and (= x s1_o6) (= y s1))
(and (= x s1_o5) (= y s1))
(and (= x s1_o4) (= y s1))
(and (= x t2_o5) (= y t2))
(and (= x t2_o4) (= y t2))
(and (= x t2_o7) (= y t2))
(and (= x t2_o6) (= y t2))
(and (= x t2_o1) (= y t2))
(and (= x t2_o3) (= y t2))
(and (= x t2_o2) (= y t2))
(and (= x t2_o9) (= y t2))
(and (= x t2_o8) (= y t2))
(and (= x t2_o15) (= y t2))
(and (= x t2_o14) (= y t2))
(and (= x t2_o11) (= y t2))
(and (= x t2_o10) (= y t2))
(and (= x t2_o13) (= y t2))
(and (= x t2_o12) (= y t2))
(and (= x t3_o4) (= y t3))
(and (= x t3_o2) (= y t3))
(and (= x t3_o3) (= y t3))
(and (= x t3_o1) (= y t3))
(and (= x t1_o1) (= y t1))
(and (= x t1_o2) (= y t1))
(and (= x t1_o3) (= y t1))
(and (= x t1_o4) (= y t1))
(and (= x t1_o5) (= y t1))
(and (= x t1_o6) (= y t1))
(and (= x t1_o7) (= y t1))
(and (= x t1_o8) (= y t1))
(and (= x t1_o9) (= y t1))
(and (= x t1_o12) (= y t1))
(and (= x t1_o13) (= y t1))
(and (= x t1_o10) (= y t1))
(and (= x t1_o11) (= y t1))
(and (= x t1_o16) (= y t1))
(and (= x t1_o14) (= y t1))
(and (= x t1_o15) (= y t1))
))
;Machinery for looking up labels of objects
;Default value is 'nullobj'
(define-fun label ((x Obj)) Obj
(ite (= x t1_o15) person
(ite (= x t1_o14) person
(ite (= x t1_o16) person
(ite (= x t1_o11) person
(ite (= x t1_o10) person
(ite (= x t1_o13) person
(ite (= x t1_o12) person
(ite (= x t2_o12) person
(ite (= x t2_o13) person
(ite (= x t2_o10) person
(ite (= x t2_o11) person
(ite (= x t2_o14) person
(ite (= x t2_o15) person
(ite (= x s1_o6) suitcase
(ite (= x s2_o1) bus
(ite (= x s1_o7) suitcase
(ite (= x s2_o2) person
(ite (= x s1_o1) suitcase
(ite (= x s1_o4) suitcase
(ite (= x s3_o2) car
(ite (= x s3_o1) bus
(ite (= x s1_o14) person
(ite (= x s1_o15) person
(ite (= x s1_o16) person
(ite (= x s1_o17) person
(ite (= x s1_o10) bus
(ite (= x s1_o11) car
(ite (= x t1_o9) person
(ite (= x t1_o8) person
(ite (= x t1_o7) person
(ite (= x t1_o6) person
(ite (= x t1_o5) person
(ite (= x t1_o4) person
(ite (= x t1_o3) car
(ite (= x t1_o2) bus
(ite (= x t1_o1) backpack
(ite (= x t3_o1) bus
(ite (= x s1_o5) suitcase
(ite (= x t3_o3) car
(ite (= x t3_o2) car
(ite (= x t3_o4) person
(ite (= x s1_o2) suitcase
(ite (= x s1_o3) suitcase
(ite (= x s1_o12) person
(ite (= x s1_o8) suitcase
(ite (= x s1_o9) truck
(ite (= x s1_o13) person
(ite (= x s1_o18) person
(ite (= x t2_o8) person
(ite (= x t2_o9) person
(ite (= x t2_o2) bus
(ite (= x t2_o3) bus
(ite (= x t2_o1) handbag
(ite (= x t2_o6) person
(ite (= x t2_o7) person
(ite (= x t2_o4) person
(ite (= x t2_o5) person
nullobj)))))))))))))))))))))))))))))))))))))))))))))))))))))))))
)
;Datatype of relations--made into a uniform arity
(declare-datatypes () ((Rels neq labelOf sameLabel toLeft toRight above below within)))
;Relation evaluator
;All relations are made into a uniform arity using 'nullobj' as a default argument
(define-fun re ((r Rels)(a Obj)(b Obj)) Bool
(or
(and (= r neq) (not (= b a)))
(and (= r labelOf) (= b (label a)))
(and (= r sameLabel) (= (label b) (label a)))
(and (= r toLeft) (= a s3_o1) (= b s3_o2))
(and (= r toLeft) (= a s1_o18) (= b s1_o11))
(and (= r toLeft) (= a s1_o18) (= b s1_o10))
(and (= r toLeft) (= a s1_o18) (= b s1_o17))
(and (= r toLeft) (= a s1_o18) (= b s1_o16))
(and (= r toLeft) (= a s1_o18) (= b s1_o15))
(and (= r toLeft) (= a s1_o18) (= b s1_o14))
(and (= r toLeft) (= a s1_o18) (= b s1_o9))
(and (= r toLeft) (= a s1_o18) (= b s1_o8))
(and (= r toLeft) (= a s1_o18) (= b s1_o3))
(and (= r toLeft) (= a s1_o18) (= b s1_o2))
(and (= r toLeft) (= a s1_o18) (= b s1_o1))
(and (= r toLeft) (= a s1_o18) (= b s1_o7))
(and (= r toLeft) (= a s1_o18) (= b s1_o6))
(and (= r toLeft) (= a s1_o18) (= b s1_o5))
(and (= r toLeft) (= a s1_o18) (= b s1_o4))
(and (= r toLeft) (= a s1_o13) (= b s1_o11))
(and (= r toLeft) (= a s1_o13) (= b s1_o10))
(and (= r toLeft) (= a s1_o13) (= b s1_o17))
(and (= r toLeft) (= a s1_o13) (= b s1_o16))
(and (= r toLeft) (= a s1_o13) (= b s1_o15))
(and (= r toLeft) (= a s1_o13) (= b s1_o14))
(and (= r toLeft) (= a s1_o13) (= b s1_o9))
(and (= r toLeft) (= a s1_o13) (= b s1_o8))
(and (= r toLeft) (= a s1_o13) (= b s1_o3))
(and (= r toLeft) (= a s1_o13) (= b s1_o2))
(and (= r toLeft) (= a s1_o13) (= b s1_o1))
(and (= r toLeft) (= a s1_o13) (= b s1_o7))
(and (= r toLeft) (= a s1_o13) (= b s1_o6))
(and (= r toLeft) (= a s1_o13) (= b s1_o5))
(and (= r toLeft) (= a s1_o13) (= b s1_o4))
(and (= r toLeft) (= a s1_o12) (= b s1_o13))
(and (= r toLeft) (= a s1_o12) (= b s1_o11))
(and (= r toLeft) (= a s1_o12) (= b s1_o10))
(and (= r toLeft) (= a s1_o12) (= b s1_o17))
(and (= r toLeft) (= a s1_o12) (= b s1_o16))
(and (= r toLeft) (= a s1_o12) (= b s1_o15))
(and (= r toLeft) (= a s1_o12) (= b s1_o14))
(and (= r toLeft) (= a s1_o12) (= b s1_o9))
(and (= r toLeft) (= a s1_o12) (= b s1_o8))
(and (= r toLeft) (= a s1_o12) (= b s1_o3))
(and (= r toLeft) (= a s1_o12) (= b s1_o2))
(and (= r toLeft) (= a s1_o12) (= b s1_o1))
(and (= r toLeft) (= a s1_o12) (= b s1_o7))
(and (= r toLeft) (= a s1_o12) (= b s1_o6))
(and (= r toLeft) (= a s1_o12) (= b s1_o5))
(and (= r toLeft) (= a s1_o12) (= b s1_o4))
(and (= r toLeft) (= a s1_o11) (= b s1_o10))
(and (= r toLeft) (= a s1_o11) (= b s1_o9))
(and (= r toLeft) (= a s1_o11) (= b s1_o3))
(and (= r toLeft) (= a s1_o11) (= b s1_o2))
(and (= r toLeft) (= a s1_o11) (= b s1_o4))
(and (= r toLeft) (= a s1_o17) (= b s1_o9))
(and (= r toLeft) (= a s1_o17) (= b s1_o3))
(and (= r toLeft) (= a s1_o17) (= b s1_o4))
(and (= r toLeft) (= a s1_o16) (= b s1_o11))
(and (= r toLeft) (= a s1_o16) (= b s1_o10))
(and (= r toLeft) (= a s1_o16) (= b s1_o17))
(and (= r toLeft) (= a s1_o16) (= b s1_o15))
(and (= r toLeft) (= a s1_o16) (= b s1_o9))
(and (= r toLeft) (= a s1_o16) (= b s1_o3))
(and (= r toLeft) (= a s1_o16) (= b s1_o2))
(and (= r toLeft) (= a s1_o16) (= b s1_o1))
(and (= r toLeft) (= a s1_o16) (= b s1_o7))
(and (= r toLeft) (= a s1_o16) (= b s1_o5))
(and (= r toLeft) (= a s1_o16) (= b s1_o4))
(and (= r toLeft) (= a s1_o15) (= b s1_o11))
(and (= r toLeft) (= a s1_o15) (= b s1_o10))
(and (= r toLeft) (= a s1_o15) (= b s1_o17))
(and (= r toLeft) (= a s1_o15) (= b s1_o9))
(and (= r toLeft) (= a s1_o15) (= b s1_o3))
(and (= r toLeft) (= a s1_o15) (= b s1_o2))
(and (= r toLeft) (= a s1_o15) (= b s1_o7))
(and (= r toLeft) (= a s1_o15) (= b s1_o5))
(and (= r toLeft) (= a s1_o15) (= b s1_o4))
(and (= r toLeft) (= a s1_o14) (= b s1_o11))
(and (= r toLeft) (= a s1_o14) (= b s1_o10))
(and (= r toLeft) (= a s1_o14) (= b s1_o17))
(and (= r toLeft) (= a s1_o14) (= b s1_o15))
(and (= r toLeft) (= a s1_o14) (= b s1_o9))
(and (= r toLeft) (= a s1_o14) (= b s1_o3))
(and (= r toLeft) (= a s1_o14) (= b s1_o2))
(and (= r toLeft) (= a s1_o14) (= b s1_o1))
(and (= r toLeft) (= a s1_o14) (= b s1_o7))
(and (= r toLeft) (= a s1_o14) (= b s1_o6))
(and (= r toLeft) (= a s1_o14) (= b s1_o5))
(and (= r toLeft) (= a s1_o14) (= b s1_o4))
(and (= r toLeft) (= a s1_o8) (= b s1_o11))
(and (= r toLeft) (= a s1_o8) (= b s1_o10))
(and (= r toLeft) (= a s1_o8) (= b s1_o17))
(and (= r toLeft) (= a s1_o8) (= b s1_o9))
(and (= r toLeft) (= a s1_o8) (= b s1_o3))
(and (= r toLeft) (= a s1_o8) (= b s1_o2))
(and (= r toLeft) (= a s1_o8) (= b s1_o1))
(and (= r toLeft) (= a s1_o8) (= b s1_o7))
(and (= r toLeft) (= a s1_o8) (= b s1_o5))
(and (= r toLeft) (= a s1_o8) (= b s1_o4))
(and (= r toLeft) (= a s1_o3) (= b s1_o9))
(and (= r toLeft) (= a s1_o2) (= b s1_o9))
(and (= r toLeft) (= a s1_o2) (= b s1_o3))
(and (= r toLeft) (= a s1_o2) (= b s1_o4))
(and (= r toLeft) (= a s1_o1) (= b s1_o11))
(and (= r toLeft) (= a s1_o1) (= b s1_o10))
(and (= r toLeft) (= a s1_o1) (= b s1_o17))
(and (= r toLeft) (= a s1_o1) (= b s1_o9))
(and (= r toLeft) (= a s1_o1) (= b s1_o3))
(and (= r toLeft) (= a s1_o1) (= b s1_o2))
(and (= r toLeft) (= a s1_o1) (= b s1_o5))
(and (= r toLeft) (= a s1_o1) (= b s1_o4))
(and (= r toLeft) (= a s1_o7) (= b s1_o11))
(and (= r toLeft) (= a s1_o7) (= b s1_o10))
(and (= r toLeft) (= a s1_o7) (= b s1_o17))
(and (= r toLeft) (= a s1_o7) (= b s1_o9))
(and (= r toLeft) (= a s1_o7) (= b s1_o3))
(and (= r toLeft) (= a s1_o7) (= b s1_o2))
(and (= r toLeft) (= a s1_o7) (= b s1_o4))
(and (= r toLeft) (= a s1_o6) (= b s1_o11))
(and (= r toLeft) (= a s1_o6) (= b s1_o10))
(and (= r toLeft) (= a s1_o6) (= b s1_o17))
(and (= r toLeft) (= a s1_o6) (= b s1_o9))
(and (= r toLeft) (= a s1_o6) (= b s1_o3))
(and (= r toLeft) (= a s1_o6) (= b s1_o2))
(and (= r toLeft) (= a s1_o6) (= b s1_o1))
(and (= r toLeft) (= a s1_o6) (= b s1_o7))
(and (= r toLeft) (= a s1_o6) (= b s1_o5))
(and (= r toLeft) (= a s1_o6) (= b s1_o4))
(and (= r toLeft) (= a s1_o5) (= b s1_o10))
(and (= r toLeft) (= a s1_o5) (= b s1_o17))
(and (= r toLeft) (= a s1_o5) (= b s1_o9))
(and (= r toLeft) (= a s1_o5) (= b s1_o3))
(and (= r toLeft) (= a s1_o5) (= b s1_o2))
(and (= r toLeft) (= a s1_o5) (= b s1_o4))
(and (= r toLeft) (= a s1_o4) (= b s1_o9))
(and (= r toLeft) (= a t2_o5) (= b t2_o9))
(and (= r toLeft) (= a t2_o5) (= b t2_o8))
(and (= r toLeft) (= a t2_o5) (= b t2_o14))
(and (= r toLeft) (= a t2_o5) (= b t2_o10))
(and (= r toLeft) (= a t2_o5) (= b t2_o13))
(and (= r toLeft) (= a t2_o4) (= b t2_o9))
(and (= r toLeft) (= a t2_o4) (= b t2_o14))
(and (= r toLeft) (= a t2_o4) (= b t2_o10))
(and (= r toLeft) (= a t2_o4) (= b t2_o13))
(and (= r toLeft) (= a t2_o7) (= b t2_o5))
(and (= r toLeft) (= a t2_o7) (= b t2_o4))
(and (= r toLeft) (= a t2_o7) (= b t2_o9))
(and (= r toLeft) (= a t2_o7) (= b t2_o8))
(and (= r toLeft) (= a t2_o7) (= b t2_o14))
(and (= r toLeft) (= a t2_o7) (= b t2_o10))
(and (= r toLeft) (= a t2_o7) (= b t2_o13))
(and (= r toLeft) (= a t2_o7) (= b t2_o12))
(and (= r toLeft) (= a t2_o6) (= b t2_o5))
(and (= r toLeft) (= a t2_o6) (= b t2_o4))
(and (= r toLeft) (= a t2_o6) (= b t2_o1))
(and (= r toLeft) (= a t2_o6) (= b t2_o3))
(and (= r toLeft) (= a t2_o6) (= b t2_o9))
(and (= r toLeft) (= a t2_o6) (= b t2_o8))
(and (= r toLeft) (= a t2_o6) (= b t2_o15))
(and (= r toLeft) (= a t2_o6) (= b t2_o14))
(and (= r toLeft) (= a t2_o6) (= b t2_o10))
(and (= r toLeft) (= a t2_o6) (= b t2_o13))
(and (= r toLeft) (= a t2_o6) (= b t2_o12))
(and (= r toLeft) (= a t2_o1) (= b t2_o5))
(and (= r toLeft) (= a t2_o1) (= b t2_o4))
(and (= r toLeft) (= a t2_o1) (= b t2_o9))
(and (= r toLeft) (= a t2_o1) (= b t2_o8))
(and (= r toLeft) (= a t2_o1) (= b t2_o14))
(and (= r toLeft) (= a t2_o1) (= b t2_o10))
(and (= r toLeft) (= a t2_o1) (= b t2_o13))
(and (= r toLeft) (= a t2_o1) (= b t2_o12))
(and (= r toLeft) (= a t2_o2) (= b t2_o5))
(and (= r toLeft) (= a t2_o2) (= b t2_o4))
(and (= r toLeft) (= a t2_o2) (= b t2_o7))
(and (= r toLeft) (= a t2_o2) (= b t2_o6))
(and (= r toLeft) (= a t2_o2) (= b t2_o1))
(and (= r toLeft) (= a t2_o2) (= b t2_o3))
(and (= r toLeft) (= a t2_o2) (= b t2_o9))
(and (= r toLeft) (= a t2_o2) (= b t2_o8))
(and (= r toLeft) (= a t2_o2) (= b t2_o15))
(and (= r toLeft) (= a t2_o2) (= b t2_o14))
(and (= r toLeft) (= a t2_o2) (= b t2_o11))
(and (= r toLeft) (= a t2_o2) (= b t2_o10))
(and (= r toLeft) (= a t2_o2) (= b t2_o13))
(and (= r toLeft) (= a t2_o2) (= b t2_o12))
(and (= r toLeft) (= a t2_o9) (= b t2_o14))
(and (= r toLeft) (= a t2_o9) (= b t2_o10))
(and (= r toLeft) (= a t2_o9) (= b t2_o13))
(and (= r toLeft) (= a t2_o8) (= b t2_o14))
(and (= r toLeft) (= a t2_o8) (= b t2_o10))
(and (= r toLeft) (= a t2_o8) (= b t2_o13))
(and (= r toLeft) (= a t2_o15) (= b t2_o5))
(and (= r toLeft) (= a t2_o15) (= b t2_o4))
(and (= r toLeft) (= a t2_o15) (= b t2_o9))
(and (= r toLeft) (= a t2_o15) (= b t2_o8))
(and (= r toLeft) (= a t2_o15) (= b t2_o14))
(and (= r toLeft) (= a t2_o15) (= b t2_o10))
(and (= r toLeft) (= a t2_o15) (= b t2_o13))
(and (= r toLeft) (= a t2_o15) (= b t2_o12))
(and (= r toLeft) (= a t2_o11) (= b t2_o5))
(and (= r toLeft) (= a t2_o11) (= b t2_o4))
(and (= r toLeft) (= a t2_o11) (= b t2_o7))
(and (= r toLeft) (= a t2_o11) (= b t2_o1))
(and (= r toLeft) (= a t2_o11) (= b t2_o3))
(and (= r toLeft) (= a t2_o11) (= b t2_o9))
(and (= r toLeft) (= a t2_o11) (= b t2_o8))
(and (= r toLeft) (= a t2_o11) (= b t2_o15))
(and (= r toLeft) (= a t2_o11) (= b t2_o14))
(and (= r toLeft) (= a t2_o11) (= b t2_o10))
(and (= r toLeft) (= a t2_o11) (= b t2_o13))
(and (= r toLeft) (= a t2_o11) (= b t2_o12))
(and (= r toLeft) (= a t2_o10) (= b t2_o14))
(and (= r toLeft) (= a t2_o10) (= b t2_o13))
(and (= r toLeft) (= a t2_o12) (= b t2_o9))
(and (= r toLeft) (= a t2_o12) (= b t2_o8))
(and (= r toLeft) (= a t2_o12) (= b t2_o14))
(and (= r toLeft) (= a t2_o12) (= b t2_o10))
(and (= r toLeft) (= a t2_o12) (= b t2_o13))
(and (= r toLeft) (= a t3_o2) (= b t3_o4))
(and (= r toLeft) (= a t3_o3) (= b t3_o4))
(and (= r toLeft) (= a t3_o3) (= b t3_o2))
(and (= r toLeft) (= a t3_o3) (= b t3_o1))
(and (= r toLeft) (= a t1_o1) (= b t1_o2))
(and (= r toLeft) (= a t1_o1) (= b t1_o3))
(and (= r toLeft) (= a t1_o1) (= b t1_o6))
(and (= r toLeft) (= a t1_o1) (= b t1_o7))
(and (= r toLeft) (= a t1_o1) (= b t1_o12))
(and (= r toLeft) (= a t1_o1) (= b t1_o10))
(and (= r toLeft) (= a t1_o1) (= b t1_o11))
(and (= r toLeft) (= a t1_o1) (= b t1_o14))
(and (= r toLeft) (= a t1_o1) (= b t1_o15))
(and (= r toLeft) (= a t1_o2) (= b t1_o3))
(and (= r toLeft) (= a t1_o4) (= b t1_o1))
(and (= r toLeft) (= a t1_o4) (= b t1_o2))
(and (= r toLeft) (= a t1_o4) (= b t1_o3))
(and (= r toLeft) (= a t1_o4) (= b t1_o5))
(and (= r toLeft) (= a t1_o4) (= b t1_o6))
(and (= r toLeft) (= a t1_o4) (= b t1_o7))
(and (= r toLeft) (= a t1_o4) (= b t1_o9))
(and (= r toLeft) (= a t1_o4) (= b t1_o12))
(and (= r toLeft) (= a t1_o4) (= b t1_o13))
(and (= r toLeft) (= a t1_o4) (= b t1_o10))
(and (= r toLeft) (= a t1_o4) (= b t1_o11))
(and (= r toLeft) (= a t1_o4) (= b t1_o16))
(and (= r toLeft) (= a t1_o4) (= b t1_o14))
(and (= r toLeft) (= a t1_o4) (= b t1_o15))
(and (= r toLeft) (= a t1_o5) (= b t1_o2))
(and (= r toLeft) (= a t1_o5) (= b t1_o3))
(and (= r toLeft) (= a t1_o5) (= b t1_o6))
(and (= r toLeft) (= a t1_o5) (= b t1_o7))
(and (= r toLeft) (= a t1_o5) (= b t1_o12))
(and (= r toLeft) (= a t1_o5) (= b t1_o10))
(and (= r toLeft) (= a t1_o5) (= b t1_o11))
(and (= r toLeft) (= a t1_o5) (= b t1_o14))
(and (= r toLeft) (= a t1_o5) (= b t1_o15))
(and (= r toLeft) (= a t1_o6) (= b t1_o3))
(and (= r toLeft) (= a t1_o6) (= b t1_o7))
(and (= r toLeft) (= a t1_o6) (= b t1_o12))
(and (= r toLeft) (= a t1_o7) (= b t1_o3))
(and (= r toLeft) (= a t1_o8) (= b t1_o1))
(and (= r toLeft) (= a t1_o8) (= b t1_o2))
(and (= r toLeft) (= a t1_o8) (= b t1_o3))
(and (= r toLeft) (= a t1_o8) (= b t1_o5))
(and (= r toLeft) (= a t1_o8) (= b t1_o6))
(and (= r toLeft) (= a t1_o8) (= b t1_o7))
(and (= r toLeft) (= a t1_o8) (= b t1_o9))
(and (= r toLeft) (= a t1_o8) (= b t1_o12))
(and (= r toLeft) (= a t1_o8) (= b t1_o10))
(and (= r toLeft) (= a t1_o8) (= b t1_o11))
(and (= r toLeft) (= a t1_o8) (= b t1_o16))
(and (= r toLeft) (= a t1_o8) (= b t1_o14))
(and (= r toLeft) (= a t1_o8) (= b t1_o15))
(and (= r toLeft) (= a t1_o9) (= b t1_o1))
(and (= r toLeft) (= a t1_o9) (= b t1_o2))
(and (= r toLeft) (= a t1_o9) (= b t1_o3))
(and (= r toLeft) (= a t1_o9) (= b t1_o6))
(and (= r toLeft) (= a t1_o9) (= b t1_o7))
(and (= r toLeft) (= a t1_o9) (= b t1_o12))
(and (= r toLeft) (= a t1_o9) (= b t1_o10))
(and (= r toLeft) (= a t1_o9) (= b t1_o11))
(and (= r toLeft) (= a t1_o9) (= b t1_o16))
(and (= r toLeft) (= a t1_o9) (= b t1_o14))
(and (= r toLeft) (= a t1_o9) (= b t1_o15))
(and (= r toLeft) (= a t1_o12) (= b t1_o3))
(and (= r toLeft) (= a t1_o12) (= b t1_o7))
(and (= r toLeft) (= a t1_o13) (= b t1_o1))
(and (= r toLeft) (= a t1_o13) (= b t1_o2))
(and (= r toLeft) (= a t1_o13) (= b t1_o3))
(and (= r toLeft) (= a t1_o13) (= b t1_o5))
(and (= r toLeft) (= a t1_o13) (= b t1_o6))
(and (= r toLeft) (= a t1_o13) (= b t1_o7))
(and (= r toLeft) (= a t1_o13) (= b t1_o12))
(and (= r toLeft) (= a t1_o13) (= b t1_o10))
(and (= r toLeft) (= a t1_o13) (= b t1_o11))
(and (= r toLeft) (= a t1_o13) (= b t1_o16))
(and (= r toLeft) (= a t1_o13) (= b t1_o14))
(and (= r toLeft) (= a t1_o13) (= b t1_o15))
(and (= r toLeft) (= a t1_o10) (= b t1_o2))
(and (= r toLeft) (= a t1_o10) (= b t1_o3))
(and (= r toLeft) (= a t1_o10) (= b t1_o6))
(and (= r toLeft) (= a t1_o10) (= b t1_o7))
(and (= r toLeft) (= a t1_o10) (= b t1_o12))
(and (= r toLeft) (= a t1_o11) (= b t1_o2))
(and (= r toLeft) (= a t1_o11) (= b t1_o3))
(and (= r toLeft) (= a t1_o11) (= b t1_o7))
(and (= r toLeft) (= a t1_o11) (= b t1_o12))
(and (= r toLeft) (= a t1_o16) (= b t1_o2))
(and (= r toLeft) (= a t1_o16) (= b t1_o3))
(and (= r toLeft) (= a t1_o16) (= b t1_o6))
(and (= r toLeft) (= a t1_o16) (= b t1_o7))
(and (= r toLeft) (= a t1_o16) (= b t1_o12))
(and (= r toLeft) (= a t1_o16) (= b t1_o10))
(and (= r toLeft) (= a t1_o16) (= b t1_o11))
(and (= r toLeft) (= a t1_o16) (= b t1_o14))
(and (= r toLeft) (= a t1_o16) (= b t1_o15))
(and (= r toLeft) (= a t1_o14) (= b t1_o2))
(and (= r toLeft) (= a t1_o14) (= b t1_o3))
(and (= r toLeft) (= a t1_o14) (= b t1_o6))
(and (= r toLeft) (= a t1_o14) (= b t1_o7))
(and (= r toLeft) (= a t1_o14) (= b t1_o12))
(and (= r toLeft) (= a t1_o14) (= b t1_o11))
(and (= r toLeft) (= a t1_o14) (= b t1_o15))
(and (= r toLeft) (= a t1_o15) (= b t1_o2))
(and (= r toLeft) (= a t1_o15) (= b t1_o3))
(and (= r toLeft) (= a t1_o15) (= b t1_o6))
(and (= r toLeft) (= a t1_o15) (= b t1_o7))
(and (= r toLeft) (= a t1_o15) (= b t1_o12))

(and (= r toRight) (= a s3_o2) (= b s3_o1))
(and (= r toRight) (= a s1_o13) (= b s1_o12))
(and (= r toRight) (= a s1_o11) (= b s1_o18))
(and (= r toRight) (= a s1_o11) (= b s1_o13))
(and (= r toRight) (= a s1_o11) (= b s1_o12))
(and (= r toRight) (= a s1_o11) (= b s1_o16))
(and (= r toRight) (= a s1_o11) (= b s1_o15))
(and (= r toRight) (= a s1_o11) (= b s1_o14))
(and (= r toRight) (= a s1_o11) (= b s1_o8))
(and (= r toRight) (= a s1_o11) (= b s1_o1))
(and (= r toRight) (= a s1_o11) (= b s1_o7))
(and (= r toRight) (= a s1_o11) (= b s1_o6))
(and (= r toRight) (= a s1_o10) (= b s1_o18))
(and (= r toRight) (= a s1_o10) (= b s1_o13))
(and (= r toRight) (= a s1_o10) (= b s1_o12))
(and (= r toRight) (= a s1_o10) (= b s1_o11))
(and (= r toRight) (= a s1_o10) (= b s1_o16))
(and (= r toRight) (= a s1_o10) (= b s1_o15))
(and (= r toRight) (= a s1_o10) (= b s1_o14))
(and (= r toRight) (= a s1_o10) (= b s1_o8))
(and (= r toRight) (= a s1_o10) (= b s1_o1))
(and (= r toRight) (= a s1_o10) (= b s1_o7))
(and (= r toRight) (= a s1_o10) (= b s1_o6))
(and (= r toRight) (= a s1_o10) (= b s1_o5))
(and (= r toRight) (= a s1_o17) (= b s1_o18))
(and (= r toRight) (= a s1_o17) (= b s1_o13))
(and (= r toRight) (= a s1_o17) (= b s1_o12))
(and (= r toRight) (= a s1_o17) (= b s1_o16))
(and (= r toRight) (= a s1_o17) (= b s1_o15))
(and (= r toRight) (= a s1_o17) (= b s1_o14))
(and (= r toRight) (= a s1_o17) (= b s1_o8))
(and (= r toRight) (= a s1_o17) (= b s1_o1))
(and (= r toRight) (= a s1_o17) (= b s1_o7))
(and (= r toRight) (= a s1_o17) (= b s1_o6))
(and (= r toRight) (= a s1_o17) (= b s1_o5))
(and (= r toRight) (= a s1_o16) (= b s1_o18))
(and (= r toRight) (= a s1_o16) (= b s1_o13))
(and (= r toRight) (= a s1_o16) (= b s1_o12))
(and (= r toRight) (= a s1_o15) (= b s1_o18))
(and (= r toRight) (= a s1_o15) (= b s1_o13))
(and (= r toRight) (= a s1_o15) (= b s1_o12))
(and (= r toRight) (= a s1_o15) (= b s1_o16))
(and (= r toRight) (= a s1_o15) (= b s1_o14))
(and (= r toRight) (= a s1_o14) (= b s1_o18))
(and (= r toRight) (= a s1_o14) (= b s1_o13))
(and (= r toRight) (= a s1_o14) (= b s1_o12))
(and (= r toRight) (= a s1_o9) (= b s1_o18))
(and (= r toRight) (= a s1_o9) (= b s1_o13))
(and (= r toRight) (= a s1_o9) (= b s1_o12))
(and (= r toRight) (= a s1_o9) (= b s1_o11))
(and (= r toRight) (= a s1_o9) (= b s1_o17))
(and (= r toRight) (= a s1_o9) (= b s1_o16))
(and (= r toRight) (= a s1_o9) (= b s1_o15))
(and (= r toRight) (= a s1_o9) (= b s1_o14))
(and (= r toRight) (= a s1_o9) (= b s1_o8))
(and (= r toRight) (= a s1_o9) (= b s1_o3))
(and (= r toRight) (= a s1_o9) (= b s1_o2))
(and (= r toRight) (= a s1_o9) (= b s1_o1))
(and (= r toRight) (= a s1_o9) (= b s1_o7))
(and (= r toRight) (= a s1_o9) (= b s1_o6))
(and (= r toRight) (= a s1_o9) (= b s1_o5))
(and (= r toRight) (= a s1_o9) (= b s1_o4))
(and (= r toRight) (= a s1_o8) (= b s1_o18))
(and (= r toRight) (= a s1_o8) (= b s1_o13))
(and (= r toRight) (= a s1_o8) (= b s1_o12))
(and (= r toRight) (= a s1_o3) (= b s1_o18))
(and (= r toRight) (= a s1_o3) (= b s1_o13))
(and (= r toRight) (= a s1_o3) (= b s1_o12))
(and (= r toRight) (= a s1_o3) (= b s1_o11))
(and (= r toRight) (= a s1_o3) (= b s1_o17))
(and (= r toRight) (= a s1_o3) (= b s1_o16))
(and (= r toRight) (= a s1_o3) (= b s1_o15))
(and (= r toRight) (= a s1_o3) (= b s1_o14))
(and (= r toRight) (= a s1_o3) (= b s1_o8))
(and (= r toRight) (= a s1_o3) (= b s1_o2))
(and (= r toRight) (= a s1_o3) (= b s1_o1))
(and (= r toRight) (= a s1_o3) (= b s1_o7))
(and (= r toRight) (= a s1_o3) (= b s1_o6))
(and (= r toRight) (= a s1_o3) (= b s1_o5))
(and (= r toRight) (= a s1_o2) (= b s1_o18))
(and (= r toRight) (= a s1_o2) (= b s1_o13))
(and (= r toRight) (= a s1_o2) (= b s1_o12))
(and (= r toRight) (= a s1_o2) (= b s1_o11))
(and (= r toRight) (= a s1_o2) (= b s1_o16))
(and (= r toRight) (= a s1_o2) (= b s1_o15))
(and (= r toRight) (= a s1_o2) (= b s1_o14))
(and (= r toRight) (= a s1_o2) (= b s1_o8))
(and (= r toRight) (= a s1_o2) (= b s1_o1))
(and (= r toRight) (= a s1_o2) (= b s1_o7))
(and (= r toRight) (= a s1_o2) (= b s1_o6))
(and (= r toRight) (= a s1_o2) (= b s1_o5))
(and (= r toRight) (= a s1_o1) (= b s1_o18))
(and (= r toRight) (= a s1_o1) (= b s1_o13))
(and (= r toRight) (= a s1_o1) (= b s1_o12))
(and (= r toRight) (= a s1_o1) (= b s1_o16))
(and (= r toRight) (= a s1_o1) (= b s1_o14))
(and (= r toRight) (= a s1_o1) (= b s1_o8))
(and (= r toRight) (= a s1_o1) (= b s1_o6))
(and (= r toRight) (= a s1_o7) (= b s1_o18))
(and (= r toRight) (= a s1_o7) (= b s1_o13))
(and (= r toRight) (= a s1_o7) (= b s1_o12))
(and (= r toRight) (= a s1_o7) (= b s1_o16))
(and (= r toRight) (= a s1_o7) (= b s1_o15))
(and (= r toRight) (= a s1_o7) (= b s1_o14))
(and (= r toRight) (= a s1_o7) (= b s1_o8))
(and (= r toRight) (= a s1_o7) (= b s1_o6))
(and (= r toRight) (= a s1_o6) (= b s1_o18))
(and (= r toRight) (= a s1_o6) (= b s1_o13))
(and (= r toRight) (= a s1_o6) (= b s1_o12))
(and (= r toRight) (= a s1_o6) (= b s1_o14))
(and (= r toRight) (= a s1_o5) (= b s1_o18))
(and (= r toRight) (= a s1_o5) (= b s1_o13))
(and (= r toRight) (= a s1_o5) (= b s1_o12))
(and (= r toRight) (= a s1_o5) (= b s1_o16))
(and (= r toRight) (= a s1_o5) (= b s1_o15))
(and (= r toRight) (= a s1_o5) (= b s1_o14))
(and (= r toRight) (= a s1_o5) (= b s1_o8))
(and (= r toRight) (= a s1_o5) (= b s1_o1))
(and (= r toRight) (= a s1_o5) (= b s1_o6))
(and (= r toRight) (= a s1_o4) (= b s1_o18))
(and (= r toRight) (= a s1_o4) (= b s1_o13))
(and (= r toRight) (= a s1_o4) (= b s1_o12))
(and (= r toRight) (= a s1_o4) (= b s1_o11))
(and (= r toRight) (= a s1_o4) (= b s1_o17))
(and (= r toRight) (= a s1_o4) (= b s1_o16))
(and (= r toRight) (= a s1_o4) (= b s1_o15))
(and (= r toRight) (= a s1_o4) (= b s1_o14))
(and (= r toRight) (= a s1_o4) (= b s1_o8))
(and (= r toRight) (= a s1_o4) (= b s1_o2))
(and (= r toRight) (= a s1_o4) (= b s1_o1))
(and (= r toRight) (= a s1_o4) (= b s1_o7))
(and (= r toRight) (= a s1_o4) (= b s1_o6))
(and (= r toRight) (= a s1_o4) (= b s1_o5))
(and (= r toRight) (= a t2_o5) (= b t2_o7))
(and (= r toRight) (= a t2_o5) (= b t2_o6))
(and (= r toRight) (= a t2_o5) (= b t2_o1))
(and (= r toRight) (= a t2_o5) (= b t2_o2))
(and (= r toRight) (= a t2_o5) (= b t2_o15))
(and (= r toRight) (= a t2_o5) (= b t2_o11))
(and (= r toRight) (= a t2_o4) (= b t2_o7))
(and (= r toRight) (= a t2_o4) (= b t2_o6))
(and (= r toRight) (= a t2_o4) (= b t2_o1))
(and (= r toRight) (= a t2_o4) (= b t2_o2))
(and (= r toRight) (= a t2_o4) (= b t2_o15))
(and (= r toRight) (= a t2_o4) (= b t2_o11))
(and (= r toRight) (= a t2_o7) (= b t2_o2))
(and (= r toRight) (= a t2_o7) (= b t2_o11))
(and (= r toRight) (= a t2_o6) (= b t2_o2))
(and (= r toRight) (= a t2_o1) (= b t2_o6))
(and (= r toRight) (= a t2_o1) (= b t2_o2))
(and (= r toRight) (= a t2_o1) (= b t2_o11))
(and (= r toRight) (= a t2_o3) (= b t2_o6))
(and (= r toRight) (= a t2_o3) (= b t2_o2))
(and (= r toRight) (= a t2_o3) (= b t2_o11))
(and (= r toRight) (= a t2_o9) (= b t2_o5))
(and (= r toRight) (= a t2_o9) (= b t2_o4))
(and (= r toRight) (= a t2_o9) (= b t2_o7))
(and (= r toRight) (= a t2_o9) (= b t2_o6))
(and (= r toRight) (= a t2_o9) (= b t2_o1))
(and (= r toRight) (= a t2_o9) (= b t2_o2))
(and (= r toRight) (= a t2_o9) (= b t2_o15))
(and (= r toRight) (= a t2_o9) (= b t2_o11))
(and (= r toRight) (= a t2_o9) (= b t2_o12))
(and (= r toRight) (= a t2_o8) (= b t2_o5))
(and (= r toRight) (= a t2_o8) (= b t2_o7))
(and (= r toRight) (= a t2_o8) (= b t2_o6))
(and (= r toRight) (= a t2_o8) (= b t2_o1))
(and (= r toRight) (= a t2_o8) (= b t2_o2))
(and (= r toRight) (= a t2_o8) (= b t2_o15))
(and (= r toRight) (= a t2_o8) (= b t2_o11))
(and (= r toRight) (= a t2_o8) (= b t2_o12))
(and (= r toRight) (= a t2_o15) (= b t2_o6))
(and (= r toRight) (= a t2_o15) (= b t2_o2))
(and (= r toRight) (= a t2_o15) (= b t2_o11))
(and (= r toRight) (= a t2_o14) (= b t2_o5))
(and (= r toRight) (= a t2_o14) (= b t2_o4))
(and (= r toRight) (= a t2_o14) (= b t2_o7))
(and (= r toRight) (= a t2_o14) (= b t2_o6))
(and (= r toRight) (= a t2_o14) (= b t2_o1))
(and (= r toRight) (= a t2_o14) (= b t2_o2))
(and (= r toRight) (= a t2_o14) (= b t2_o9))
(and (= r toRight) (= a t2_o14) (= b t2_o8))
(and (= r toRight) (= a t2_o14) (= b t2_o15))
(and (= r toRight) (= a t2_o14) (= b t2_o11))
(and (= r toRight) (= a t2_o14) (= b t2_o10))
(and (= r toRight) (= a t2_o14) (= b t2_o12))
(and (= r toRight) (= a t2_o11) (= b t2_o2))
(and (= r toRight) (= a t2_o10) (= b t2_o5))
(and (= r toRight) (= a t2_o10) (= b t2_o4))
(and (= r toRight) (= a t2_o10) (= b t2_o7))
(and (= r toRight) (= a t2_o10) (= b t2_o6))
(and (= r toRight) (= a t2_o10) (= b t2_o1))
(and (= r toRight) (= a t2_o10) (= b t2_o2))
(and (= r toRight) (= a t2_o10) (= b t2_o9))
(and (= r toRight) (= a t2_o10) (= b t2_o8))
(and (= r toRight) (= a t2_o10) (= b t2_o15))
(and (= r toRight) (= a t2_o10) (= b t2_o11))
(and (= r toRight) (= a t2_o10) (= b t2_o12))
(and (= r toRight) (= a t2_o13) (= b t2_o5))
(and (= r toRight) (= a t2_o13) (= b t2_o4))
(and (= r toRight) (= a t2_o13) (= b t2_o7))
(and (= r toRight) (= a t2_o13) (= b t2_o6))
(and (= r toRight) (= a t2_o13) (= b t2_o1))
(and (= r toRight) (= a t2_o13) (= b t2_o2))
(and (= r toRight) (= a t2_o13) (= b t2_o9))
(and (= r toRight) (= a t2_o13) (= b t2_o8))
(and (= r toRight) (= a t2_o13) (= b t2_o15))
(and (= r toRight) (= a t2_o13) (= b t2_o11))
(and (= r toRight) (= a t2_o13) (= b t2_o10))
(and (= r toRight) (= a t2_o13) (= b t2_o12))
(and (= r toRight) (= a t2_o12) (= b t2_o7))
(and (= r toRight) (= a t2_o12) (= b t2_o6))
(and (= r toRight) (= a t2_o12) (= b t2_o1))
(and (= r toRight) (= a t2_o12) (= b t2_o2))
(and (= r toRight) (= a t2_o12) (= b t2_o15))
(and (= r toRight) (= a t2_o12) (= b t2_o11))
(and (= r toRight) (= a t3_o4) (= b t3_o2))
(and (= r toRight) (= a t3_o4) (= b t3_o3))
(and (= r toRight) (= a t3_o2) (= b t3_o3))
(and (= r toRight) (= a t3_o1) (= b t3_o3))
(and (= r toRight) (= a t1_o1) (= b t1_o4))
(and (= r toRight) (= a t1_o1) (= b t1_o8))
(and (= r toRight) (= a t1_o1) (= b t1_o9))
(and (= r toRight) (= a t1_o1) (= b t1_o13))
(and (= r toRight) (= a t1_o2) (= b t1_o1))
(and (= r toRight) (= a t1_o2) (= b t1_o4))
(and (= r toRight) (= a t1_o2) (= b t1_o5))
(and (= r toRight) (= a t1_o2) (= b t1_o8))
(and (= r toRight) (= a t1_o2) (= b t1_o9))
(and (= r toRight) (= a t1_o2) (= b t1_o13))
(and (= r toRight) (= a t1_o2) (= b t1_o10))
(and (= r toRight) (= a t1_o2) (= b t1_o11))
(and (= r toRight) (= a t1_o2) (= b t1_o16))
(and (= r toRight) (= a t1_o2) (= b t1_o14))
(and (= r toRight) (= a t1_o2) (= b t1_o15))
(and (= r toRight) (= a t1_o3) (= b t1_o1))
(and (= r toRight) (= a t1_o3) (= b t1_o2))
(and (= r toRight) (= a t1_o3) (= b t1_o4))
(and (= r toRight) (= a t1_o3) (= b t1_o5))
(and (= r toRight) (= a t1_o3) (= b t1_o6))
(and (= r toRight) (= a t1_o3) (= b t1_o7))
(and (= r toRight) (= a t1_o3) (= b t1_o8))
(and (= r toRight) (= a t1_o3) (= b t1_o9))
(and (= r toRight) (= a t1_o3) (= b t1_o12))
(and (= r toRight) (= a t1_o3) (= b t1_o13))
(and (= r toRight) (= a t1_o3) (= b t1_o10))
(and (= r toRight) (= a t1_o3) (= b t1_o11))
(and (= r toRight) (= a t1_o3) (= b t1_o16))
(and (= r toRight) (= a t1_o3) (= b t1_o14))
(and (= r toRight) (= a t1_o3) (= b t1_o15))
(and (= r toRight) (= a t1_o5) (= b t1_o4))
(and (= r toRight) (= a t1_o5) (= b t1_o8))
(and (= r toRight) (= a t1_o5) (= b t1_o13))
(and (= r toRight) (= a t1_o6) (= b t1_o1))
(and (= r toRight) (= a t1_o6) (= b t1_o4))
(and (= r toRight) (= a t1_o6) (= b t1_o5))
(and (= r toRight) (= a t1_o6) (= b t1_o8))
(and (= r toRight) (= a t1_o6) (= b t1_o9))
(and (= r toRight) (= a t1_o6) (= b t1_o13))
(and (= r toRight) (= a t1_o6) (= b t1_o10))
(and (= r toRight) (= a t1_o6) (= b t1_o16))
(and (= r toRight) (= a t1_o6) (= b t1_o14))
(and (= r toRight) (= a t1_o6) (= b t1_o15))
(and (= r toRight) (= a t1_o7) (= b t1_o1))
(and (= r toRight) (= a t1_o7) (= b t1_o4))
(and (= r toRight) (= a t1_o7) (= b t1_o5))
(and (= r toRight) (= a t1_o7) (= b t1_o6))
(and (= r toRight) (= a t1_o7) (= b t1_o8))
(and (= r toRight) (= a t1_o7) (= b t1_o9))
(and (= r toRight) (= a t1_o7) (= b t1_o12))
(and (= r toRight) (= a t1_o7) (= b t1_o13))
(and (= r toRight) (= a t1_o7) (= b t1_o10))
(and (= r toRight) (= a t1_o7) (= b t1_o11))
(and (= r toRight) (= a t1_o7) (= b t1_o16))
(and (= r toRight) (= a t1_o7) (= b t1_o14))
(and (= r toRight) (= a t1_o7) (= b t1_o15))
(and (= r toRight) (= a t1_o9) (= b t1_o4))
(and (= r toRight) (= a t1_o9) (= b t1_o8))
(and (= r toRight) (= a t1_o12) (= b t1_o1))
(and (= r toRight) (= a t1_o12) (= b t1_o4))
(and (= r toRight) (= a t1_o12) (= b t1_o5))
(and (= r toRight) (= a t1_o12) (= b t1_o6))
(and (= r toRight) (= a t1_o12) (= b t1_o8))
(and (= r toRight) (= a t1_o12) (= b t1_o9))
(and (= r toRight) (= a t1_o12) (= b t1_o13))
(and (= r toRight) (= a t1_o12) (= b t1_o10))
(and (= r toRight) (= a t1_o12) (= b t1_o11))
(and (= r toRight) (= a t1_o12) (= b t1_o16))
(and (= r toRight) (= a t1_o12) (= b t1_o14))
(and (= r toRight) (= a t1_o12) (= b t1_o15))
(and (= r toRight) (= a t1_o13) (= b t1_o4))
(and (= r toRight) (= a t1_o10) (= b t1_o1))
(and (= r toRight) (= a t1_o10) (= b t1_o4))
(and (= r toRight) (= a t1_o10) (= b t1_o5))
(and (= r toRight) (= a t1_o10) (= b t1_o8))
(and (= r toRight) (= a t1_o10) (= b t1_o9))
(and (= r toRight) (= a t1_o10) (= b t1_o13))
(and (= r toRight) (= a t1_o10) (= b t1_o16))
(and (= r toRight) (= a t1_o11) (= b t1_o1))
(and (= r toRight) (= a t1_o11) (= b t1_o4))
(and (= r toRight) (= a t1_o11) (= b t1_o5))
(and (= r toRight) (= a t1_o11) (= b t1_o8))
(and (= r toRight) (= a t1_o11) (= b t1_o9))
(and (= r toRight) (= a t1_o11) (= b t1_o13))
(and (= r toRight) (= a t1_o11) (= b t1_o16))
(and (= r toRight) (= a t1_o11) (= b t1_o14))
(and (= r toRight) (= a t1_o16) (= b t1_o4))
(and (= r toRight) (= a t1_o16) (= b t1_o8))
(and (= r toRight) (= a t1_o16) (= b t1_o9))
(and (= r toRight) (= a t1_o16) (= b t1_o13))
(and (= r toRight) (= a t1_o14) (= b t1_o1))
(and (= r toRight) (= a t1_o14) (= b t1_o4))
(and (= r toRight) (= a t1_o14) (= b t1_o5))
(and (= r toRight) (= a t1_o14) (= b t1_o8))
(and (= r toRight) (= a t1_o14) (= b t1_o9))
(and (= r toRight) (= a t1_o14) (= b t1_o13))
(and (= r toRight) (= a t1_o14) (= b t1_o16))
(and (= r toRight) (= a t1_o15) (= b t1_o1))
(and (= r toRight) (= a t1_o15) (= b t1_o4))
(and (= r toRight) (= a t1_o15) (= b t1_o5))
(and (= r toRight) (= a t1_o15) (= b t1_o8))
(and (= r toRight) (= a t1_o15) (= b t1_o9))
(and (= r toRight) (= a t1_o15) (= b t1_o13))
(and (= r toRight) (= a t1_o15) (= b t1_o16))
(and (= r toRight) (= a t1_o15) (= b t1_o14))

(and (= r above) (= a s1_o11) (= b s1_o8))
(and (= r above) (= a s1_o11) (= b s1_o3))
(and (= r above) (= a s1_o11) (= b s1_o2))
(and (= r above) (= a s1_o11) (= b s1_o1))
(and (= r above) (= a s1_o11) (= b s1_o7))
(and (= r above) (= a s1_o11) (= b s1_o6))
(and (= r above) (= a s1_o11) (= b s1_o4))
(and (= r above) (= a s1_o16) (= b s1_o8))
(and (= r above) (= a s1_o16) (= b s1_o3))
(and (= r above) (= a s1_o16) (= b s1_o2))
(and (= r above) (= a s1_o16) (= b s1_o1))
(and (= r above) (= a s1_o16) (= b s1_o7))
(and (= r above) (= a s1_o16) (= b s1_o4))
(and (= r above) (= a s1_o15) (= b s1_o3))
(and (= r above) (= a s1_o6) (= b s1_o3))
(and (= r above) (= a s1_o4) (= b s1_o3))
(and (= r above) (= a t2_o2) (= b t2_o4))
(and (= r above) (= a t2_o2) (= b t2_o1))
(and (= r above) (= a t2_o14) (= b t2_o5))
(and (= r above) (= a t2_o14) (= b t2_o4))
(and (= r above) (= a t2_o14) (= b t2_o7))
(and (= r above) (= a t2_o14) (= b t2_o6))
(and (= r above) (= a t2_o14) (= b t2_o1))
(and (= r above) (= a t2_o14) (= b t2_o2))
(and (= r above) (= a t2_o14) (= b t2_o9))
(and (= r above) (= a t2_o14) (= b t2_o8))
(and (= r above) (= a t2_o14) (= b t2_o15))
(and (= r above) (= a t2_o14) (= b t2_o11))
(and (= r above) (= a t2_o14) (= b t2_o10))
(and (= r above) (= a t2_o14) (= b t2_o12))
(and (= r above) (= a t2_o10) (= b t2_o1))
(and (= r above) (= a t2_o13) (= b t2_o5))
(and (= r above) (= a t2_o13) (= b t2_o4))
(and (= r above) (= a t2_o13) (= b t2_o7))
(and (= r above) (= a t2_o13) (= b t2_o6))
(and (= r above) (= a t2_o13) (= b t2_o1))
(and (= r above) (= a t2_o13) (= b t2_o2))
(and (= r above) (= a t2_o13) (= b t2_o9))
(and (= r above) (= a t2_o13) (= b t2_o8))
(and (= r above) (= a t2_o13) (= b t2_o15))
(and (= r above) (= a t2_o13) (= b t2_o11))
(and (= r above) (= a t2_o13) (= b t2_o10))
(and (= r above) (= a t2_o13) (= b t2_o12))
(and (= r above) (= a t2_o12) (= b t2_o1))
(and (= r above) (= a t3_o4) (= b t3_o2))
(and (= r above) (= a t3_o4) (= b t3_o3))
(and (= r above) (= a t1_o3) (= b t1_o1))

(and (= r below) (= a s1_o8) (= b s1_o11))
(and (= r below) (= a s1_o8) (= b s1_o16))
(and (= r below) (= a s1_o3) (= b s1_o11))
(and (= r below) (= a s1_o3) (= b s1_o16))
(and (= r below) (= a s1_o3) (= b s1_o15))
(and (= r below) (= a s1_o3) (= b s1_o6))
(and (= r below) (= a s1_o3) (= b s1_o4))
(and (= r below) (= a s1_o2) (= b s1_o11))
(and (= r below) (= a s1_o2) (= b s1_o16))
(and (= r below) (= a s1_o1) (= b s1_o11))
(and (= r below) (= a s1_o1) (= b s1_o16))
(and (= r below) (= a s1_o7) (= b s1_o11))
(and (= r below) (= a s1_o7) (= b s1_o16))
(and (= r below) (= a s1_o6) (= b s1_o11))
(and (= r below) (= a s1_o4) (= b s1_o11))
(and (= r below) (= a s1_o4) (= b s1_o16))
(and (= r below) (= a t2_o5) (= b t2_o14))
(and (= r below) (= a t2_o5) (= b t2_o13))
(and (= r below) (= a t2_o4) (= b t2_o2))
(and (= r below) (= a t2_o4) (= b t2_o14))
(and (= r below) (= a t2_o4) (= b t2_o13))
(and (= r below) (= a t2_o7) (= b t2_o14))
(and (= r below) (= a t2_o7) (= b t2_o13))
(and (= r below) (= a t2_o6) (= b t2_o14))
(and (= r below) (= a t2_o6) (= b t2_o13))
(and (= r below) (= a t2_o1) (= b t2_o2))
(and (= r below) (= a t2_o1) (= b t2_o14))
(and (= r below) (= a t2_o1) (= b t2_o10))
(and (= r below) (= a t2_o1) (= b t2_o13))
(and (= r below) (= a t2_o1) (= b t2_o12))
(and (= r below) (= a t2_o2) (= b t2_o14))
(and (= r below) (= a t2_o2) (= b t2_o13))
(and (= r below) (= a t2_o9) (= b t2_o14))
(and (= r below) (= a t2_o9) (= b t2_o13))
(and (= r below) (= a t2_o8) (= b t2_o14))
(and (= r below) (= a t2_o8) (= b t2_o13))
(and (= r below) (= a t2_o15) (= b t2_o14))
(and (= r below) (= a t2_o15) (= b t2_o13))
(and (= r below) (= a t2_o11) (= b t2_o14))
(and (= r below) (= a t2_o11) (= b t2_o13))
(and (= r below) (= a t2_o10) (= b t2_o14))
(and (= r below) (= a t2_o10) (= b t2_o13))
(and (= r below) (= a t2_o12) (= b t2_o14))
(and (= r below) (= a t2_o12) (= b t2_o13))
(and (= r below) (= a t3_o2) (= b t3_o4))
(and (= r below) (= a t3_o3) (= b t3_o4))
(and (= r below) (= a t1_o1) (= b t1_o3))

(and (= r within) (= a s2_o2) (= b s2_o1))
(and (= r within) (= a s1_o4) (= b s1_o10))
(and (= r within) (= a t2_o1) (= b t2_o3))
(and (= r within) (= a t2_o9) (= b t2_o3))
(and (= r within) (= a t2_o15) (= b t2_o3))
(and (= r within) (= a t2_o14) (= b t2_o3))
(and (= r within) (= a t2_o10) (= b t2_o3))
(and (= r within) (= a t2_o13) (= b t2_o3))
(and (= r within) (= a t2_o12) (= b t2_o3))
(and (= r within) (= a t3_o4) (= b t3_o1))
(and (= r within) (= a t1_o7) (= b t1_o2))
(and (= r within) (= a t1_o12) (= b t1_o2))

))
;Datatype for indicating which quantifier to use in formula
(declare-datatypes () ((Quantifier all one)))
;Datatype of indirections to indicate what the arguments of relations in the abstract baseformula are-- i.e, a datatype of bindings 
;Constants/extraneous objects have bindings as well
(declare-datatypes () ((Binding
 bind_x1 bind_x2
;bindings for extraneous objects
 bind_person bind_car bind_backpack bind_suitcase bind_nullobj bind_truck bind_bus bind_handbag
)))
;Defines how bindings correspond to valuations
;Default lookup is nullobj, but it is used since the function is defined for all bindings
(define-fun bindlook ((x1 Obj)(x2 Obj)(bind Binding)) Obj
(ite (= bind bind_x2) x2
(ite (= bind bind_x1) x1
;Bindings for extraneous objects
(ite (= bind bind_handbag) handbag
(ite (= bind bind_bus) bus
(ite (= bind bind_truck) truck
(ite (= bind bind_nullobj) nullobj
(ite (= bind bind_suitcase) suitcase
(ite (= bind bind_backpack) backpack
(ite (= bind bind_car) car
(ite (= bind bind_person) person
 nullobj))))))))))
)
;Defines symbols used to indicate which boolean operators will be used, including negations, and their meanings as well
(declare-datatypes () ((Ops opand opor opimpl)))
(declare-datatypes () ((IfNeg yes no)))
(define-fun opeval ((op Ops) (arg1 Bool) (arg2 Bool)) Bool
 (ite (= op opand) (and arg1 arg2)
 (ite (= op opor) (or arg1 arg2)
  (=> arg1 arg2)
)))
(define-fun negeval ((val IfNeg)(arg Bool)) Bool
 (ite (= val yes)
  (not arg)
  arg
))
(declare-const q1 Quantifier)
(declare-const q2 Quantifier)

(declare-const op1 Ops)

(declare-const n1 IfNeg)
(declare-const n2 IfNeg)

(declare-const r1 Rels)
(declare-const r2 Rels)

(declare-const arg1_1 Binding)
(declare-const arg1_2 Binding)
(declare-const arg2_1 Binding)
(declare-const arg2_2 Binding)

(assert (= q1 one))
(assert (= q2 one))

(assert (= op1 opand))

(assert (=> (= r1 labelOf) (= n1 no)))
(assert (=> (= r2 labelOf) (= n2 no)))

;Defines the innermost (quantifier-free) formula
(define-fun baseformula ((x1 Obj)(x2 Obj)) Bool
(opeval op1
(negeval n1 (re r1 (bindlook x1 x2 arg1_1) (bindlook x1 x2 arg1_2)))
(negeval n2 (re r2 (bindlook x1 x2 arg2_1) (bindlook x1 x2 arg2_2))))
)
;Formula at quantifier level 2
(define-fun formula_level_2 ((x1 Obj) (img Img)) Bool
(ite (= q2 all)
 (forall ((x2 Obj)) (=> (inImg x2 img) (baseformula x1 x2)))
 (exists ((x2 Obj)) (and (inImg x2 img) (baseformula x1 x2)))
));Formula at quantifier level 1
(define-fun formula_level_1 ((img Img)) Bool
(ite (= q1 all)
 (forall ((x1 Obj)) (=> (inImg x1 img) (formula_level_2 x1 img)))
 (exists ((x1 Obj)) (and (inImg x1 img) (formula_level_2 x1 img)))
))

(assert (formula_level_1 t2))
(assert (formula_level_1 t3))
(assert (formula_level_1 t1))

;(push)
;(echo "Results for s3")
;(assert (formula_level_1 s3))
;(assert (not (formula_level_1 s2)))
;(assert (not (formula_level_1 s1)))
;(check-sat)
;(get-model)
;(pop)

;(push)
;(echo "Results for s2")
;(assert (formula_level_1 s2))
;(assert (not (formula_level_1 s3)))
;(assert (not (formula_level_1 s1)))

;(assert (not (and 
;(= r1 labelOf)
;(= r2 within)
;)))
;(assert (not (and 
;(= r2 labelOf)
;(= r1 within)
;)))

;(check-sat)
;(get-model)
;(pop)

(push)
(echo "Results for s1")
(assert (formula_level_1 s1))
(assert (not (formula_level_1 s3)))
(assert (not (formula_level_1 s2)))

(assert (not (or 
(= r1 above)
(= r2 above)
(= r1 below)
(= r2 below)
(= r1 neq)
(= r2 neq)
)))

(assert (not (and 
(or (= r2 labelOf) (= r2 sameLabel))
(= r1 toLeft)
)))

(assert (not (and 
(or (= r2 labelOf) (= r2 sameLabel))
(= r1 toRight)
)))

(assert (not (and 
(or (= r1 labelOf) (= r1 sameLabel))
(= r2 toLeft)
)))


(assert (not (and 
(or (= r1 labelOf) (= r1 sameLabel))
(= r2 toRight)
)))
(check-sat)
(get-model)
(pop)


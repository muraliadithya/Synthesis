...
def rel_expr(index,num_vars,arity):
  varstr = ""
  ...
  result = "(negeval n" + str(index) + " (re r" + str(index) + temp + "))"
  return result
...
def annotate_parsetree(dt,num_vars,arity):
  if type(dt) == tuple:
    raise ValueError('You gave me the emtire tuple. Give me just the tree.')

  if type(dt) != list:
    return rel_expr(dt,num_vars,arity)
  else:
